...
contract MultiSigWallet {
    ...
    constructor(address[] _owners, uint _required, address _tokenAddress)
        public
        ...
        notExecuted(txId)
    {
        if (isConfirmed(txId)) {
            ...
            // transfer tokens from MultiSigWallet to receiver
            if (external_call(transactions[txId].destination, transactions[txId].value, transactions[txId].data.length, transactions[txId].data))
                emit Execution(txId);
            else {
